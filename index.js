...
async function run() {
  try {
    ...
    app.get('/find-donor', async(req, res)=>{
      ...
      const query = {role:'donor', status:'active', availability: "available",};
      const query2 = {role:'donor'};
      if(bloodGroup) query.bloodGroup = bloodGroup;
      if(district) query.district = district;
      const skip = (parseInt(page) - 1) * parseInt(limit);
      const donors = await usersCollection.find(query).skip(skip).limit(parseInt(limit)).toArray();
      const total = await usersCollection.countDocuments(query2);
      const activeTotal = await usersCollection.countDocuments(query);
      res.send({ total, activeTotal, page: parseInt(page), limit: parseInt(limit), data: donors,});
    })

    app.get('/stats', async(req, res)=>{
      const query = {role: 'donor'}
      const query2 = {role: 'donor',status:'active', availability: "available",};
    ...
    // For admin all user
    app.get('/users/allUser/by/admin', verifyToken, verifyAdmin, async(req, res)=>{
    ...
    app.patch('/users/user/status/:id', async(req, res)=>{
      ...
      const result = await usersCollection.updateOne({_id:new ObjectId(id)}, {$set:{status:newStatus}});
      res.send(result);
    })

    app.delete('/users/user/delete/:id', verifyToken, verifyAdmin, async(req, res)=>{
      const id = {_id: new ObjectId(req.params.id)};
      const result = await usersCollection.deleteOne(id);
      res.send(result);
    })

    app.get('/users/allDonor/by/admin', verifyToken, verifyAdmin, async(req, res)=>{
    ...
    app.post('/req-donor-blood/:email', async(req, res)=>{
      ...
      if (!user || !donor) {
        return res.status(404).send({ success: false, message: 'User or donor not found' });
      }
    ...
    app.get('/my-blood-requests', async (req, res) => {
      ...
      const requests = await bloodRequestCollection.find(query).sort({_id:-1}).toArray();
    ...
    // for Donor.........................
    app.get('/blood-requests/donor', verifyToken, verifyDonor, async (req, res) => {
      const userEmail = req.query.email;
